...
namespace Klli.Sensact.Config.Applications
{
    ...
    public class RgbwApplication : ActorApplication
    {
        ...
        public override string GenerateInitializer(ModelContainer m)
        {
            ...
            sb.AppendFormat("sensactapps::cRgbw {0}(eApplicationID::{0}, {1}, {2}, {3}, {4}, {5}, (uint8_t*)MODEL::wellKnownRGBWColors, MODEL::wellKnownRGBWColorsCnt, eApplicationID::{6}, {7});" + Environment.NewLine + Environment.NewLine, ApplicationId, OutputRessourceR, OutputRessourceG, OutputRessourceB, OutputRessourceW, LowMeansLampOn.ToString().ToLower(), StandbyController, AutoOffIntervalMsecs);
            return sb.ToString();
        }
