...
namespace sensactcore
{
	...
	static void USARTInitModule()
	{

		Console::Writeln(sensactstrings::GIMMICK);
		Console::Writeln("");
	...
	{
		for (size_t i = 0; i < heartbeatBuffer.size(); i++)
		{
		...
		cApplication::SendHEARTBEATCommand(target, (uint32_t)MODEL::NodeMasterApplication, &currentCtx);
	}
	...
	void cMaster::Run(INodeAsPhaseTrigger *nodePhase, INodeAsIOProvider *nodeIO)
	{
		Time_t now;
		hal->GetSteadyClock(&now);
		this->currentCtx.master=this;
		this->currentCtx.node=nodeIO;
#ifdef MASTERNODE
		MX_LWIP_Init();
		httpd_init();
		ip4_addr_t ip_addr;
		...
		while (true)
		{
			...
			hal->GetSteadyClock(&(currentCtx.now));
			...
			for (appId = 0; appId < (uint16_t)eApplicationID::CNT; appId++)
			{
				...
				if (!ap)
					continue;
				appResult = ap->DoEachCycle(&currentCtx, statusBuffer, &statusBufferLength);
#ifdef MASTERNODE
			...
			if (BufferHasMessage)
			{
				//cShell::processCmds((uint8_t *)UART_cmdBuffer, UART_buffer_pointer); //TODO Shell
			...
			for (uint8_t i = 0; i < heartbeatBuffer.size(); i++)
			{
				if (heartbeatBuffer[i] == eApplicationID::NO_APPLICATION)
					break;
				LOGD("Flushing heartbeat buffer with target %s", cApplication::N4I((uint32_t const)heartbeatBuffer[i]));
				cApplication::SendHEARTBEATCommand(heartbeatBuffer[i], (uint32_t)MODEL::NodeMasterApplication, &currentCtx);
				heartbeatBuffer[i] = eApplicationID::NO_APPLICATION;
			}
			Time_t lastNow=now;
			hal->GetSteadyClock(&now);
			while ((now - lastNow) < 20)
			{
				CanBusProcess();
				hal->GetSteadyClock(&now);
			}
 ...
 */
...
#endif
	void cMaster::OnCommand(eCommandType command, uint8_t *data, uint8_t dataLenght)
	{
		UU(command);
		UU(data);
		UU(dataLenght);
	...
	bool cMaster::SendCommandToMessageBus(eApplicationID destinationApp, eCommandType cmd,
										  const uint8_t *const payload, const uint8_t payloadLength)
	{
		...
		if (app != NULL)
		{
			...
			app->OnCommand(cmd, payload, payloadLength, &currentCtx);
			return true;
		}
			
		lastSentCANMessage = currentCtx.now;
		uint32_t canid = cCanIdUtils::CreateCommandMessageId((u16)destinationApp, (u8)cmd);
#ifdef NEW_CANID
		return haö->SendCANMessage(canid, payload, payloadLength)==Error::OK;
#else
		if (payloadLength > 7)
		{
		...
		return hal->SendCANMessage(canid, buf, payloadLength + 1)==Error::OK;
#endif	
		
	}

	void cMaster::PublishNodeEvent(eNodeID sourceNode, eNodeEventType event, uint8_t *payload, uint8_t payloadLength)
	{
		lastSentCANMessage = currentCtx.now;
		hal->SendCANMessage(cCanIdUtils::CreateNodeEventMessageId((u8)sourceNode, (u8)event), payload, payloadLength);
#ifdef MASTERNODE
		cMaster::mqtt_publishOnTopic(eMqttTopic::NODE_EVENT, payload, payloadLength, 0, 1);
#endif
	}

	void cMaster::PublishApplicationEvent(eApplicationID sourceApp, eEventType event, uint8_t *payload, uint8_t payloadLength)
	{
		lastSentCANMessage = currentCtx.now;
		hal->SendCANMessage(cCanIdUtils::CreateEventMessageId((u16)sourceApp, (u8)event), payload, payloadLength);
#ifdef MASTERNODE
		uint8_t buf[11];
	...
	void cMaster::PublishApplicationStatus(eApplicationID sourceApp, eApplicationStatus statusType, uint8_t *payload, uint8_t payloadLength)
	{
		lastSentCANMessage = currentCtx.now;
		hal->SendCANMessage(cCanIdUtils::CreateApplicationStatusMessageId((u16)sourceApp, (u8)statusType), payload, payloadLength);
#ifdef MASTERNODE
		uint8_t buf[11];
		Common::WriteInt16((u16)sourceApp, buf, 0);
		buf[2] = (u8)statusType;
	...
	void cMaster::PublishApplicationEventFiltered(const eApplicationID sourceApp, const eEventType evt, const eEventType *const localEvts, const uint8_t localEvtsLength, const eEventType *const busEvts, const uint8_t busEvtsLength, uint8_t *payload, uint8_t payloadLength)
	{
		uint8_t i;
		...
		for (i = 0; i < localEvtsLength; i++)
		{
			...
			if (evt == test)
			{
				...
				if (app != NULL)
				{
					app->OnEvent(sourceApp, evt, payload, payloadLength, &currentCtx);
				}
				break;
			}
		}
		for (i = 0; i < busEvtsLength; i++)
		{
			...
			if (evt == test)
			{
				PublishApplicationEvent(sourceApp, evt, payload, payloadLength);
				break;
			}
		}
	}
	...
	void cMaster::CanBusProcess()
	{
		...
		hal->GetSteadyClock(&now);
		cApplication *app;
		while (hal->ReceiveCANMessage(&rcvMessage))
		{
			...
			case eCanMessageType::ApplicationCommand:
				...
				{
#ifdef NEW_CANID
					app->OnCommand((eCommandType)commandOrEventId, rcvMessage.Data, rcvMessage.Length, ctx);
#else
					app->OnCommand((eCommandType)commandOrEventId, &rcvMessage.Data[1], rcvMessage.Length - 1, &currentCtx);
#endif
				}
				break;
			case eCanMessageType::ApplicationEvent:
				...
				if (app != NULL)
				{
					app->OnEvent((eApplicationID)appId, (eEventType)commandOrEventId, rcvMessage.Data, rcvMessage.Length, &currentCtx);
				}
				break;
			case eCanMessageType::ApplicationStatus:
				...
				break;
			default:
				LOGW("Unknown application type %d", type);
				break;
			}
		}
	}
/*
	...
	*/
	...
	{
		static const uint16_t tage_seit_jahresanfang[12] = /* Anzahl der Tage seit Jahresanfang ohne Tage des aktuellen Monats und ohne Schalttag */
