...
void cPWM::OnSET_VERTICAL_TARGETCommand(uint16_t target, SensactContext *ctx)
{
	LOGD("%s OnSET_VERTICAL_TARGETCommand called with  %d", N(), target);
	SetTargetAbsolute(target, ctx->now);
...
void cPWM::OnSTEP_VERTICALCommand(int16_t step, SensactContext *ctx) {


	if(targetLevel==0)
	{
		if(step>0)
			SetTargetAbsolute(minimalOnLevel, ctx->now);
		else
			SetTargetAbsolute(MAXIMUM_LEVEL, ctx->now);
	}
	else
	{
		SetTargetRelative(4*step, ctx->now);
	}
	if(autoOffIntervalMsecs!=0)
	{
		autoOffTime=ctx->now+autoOffIntervalMsecs;
...
//gesendet vom 1BP
void cPWM::OnSTARTCommand(SensactContext *ctx)
{
	LOGD("%s OnSTARTCommand called", N());
	if(ctx->now-this->lastUserSignal> TIME_TO_FORGET_DIM_DIRECTION)
	{
	...
	MoveInDirection(this->OneButtonDimDirection, ctx->now);
}

//gesendet vom 1PB
void cPWM::OnSTOPCommand(SensactContext *ctx)
{
	LOGD("%s OnSTOPCommand called", N());
	StopMove(ctx->now);
	if(autoOffIntervalMsecs!=0)
	{
		autoOffTime=ctx->now+autoOffIntervalMsecs;
...
void cPWM::OnUPCommand(uint8_t forced, SensactContext *ctx)
{
	UU(forced);
	LOGI("%s OnUPCommand called", N());
	MoveInDirection(eDirection::UP, ctx->now);
	if(autoOffIntervalMsecs!=0)
	{
		autoOffTime=ctx->now+autoOffIntervalMsecs;
	}
...
void cPWM::OnTOGGLECommand(SensactContext *ctx)
{
	if (targetLevel == 0) {
...
void cPWM::OnONCommand(uint32_t autoReturnToOffMsecs, SensactContext *ctx) {
	LOGI("%s OnONCommand called", N());
	...
	{
		autoOffTime=ctx->now+autoReturnToOffMsecs;
	}
}

void cPWM::OnDOWNCommand(uint8_t forced,  SensactContext *ctx) {
	UU(forced);
	LOGD("%s OnDOWNCommand called", N());
	MoveInDirection(eDirection::DOWN, ctx->now);
	if(autoOffIntervalMsecs!=0)
	{
		autoOffTime=ctx->now+autoOffIntervalMsecs;
	}
...
eAppCallResult cPWM::DoEachCycle(SensactContext *ctx, uint8_t *statusBuffer, size_t *statusBufferLength) {
	if(autoOffIntervalMsecs!=0 && autoOffTime<ctx->now && targetLevel>0)
	{
	...
	if(standbyController!=eApplicationID::NO_APPLICATION && currentLevel>0 && ctx->now-lastHeartbeatToStandbycontroller>3000)
	{
		LOGD("%s sends heartbeat to %s at level %d", N(), NN(standbyController), currentLevel);
		ctx->master->BufferHeartbeat(standbyController);
...
void cPWM::WriteCurrentLevelToOutput(SensactContext *ctx) {
...
eAppCallResult cPWM::Setup(SensactContext *ctx) {
	currentLevel=0;
	WriteCurrentLevelToOutput(ctx);
