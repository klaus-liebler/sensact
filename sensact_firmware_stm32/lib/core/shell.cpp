...
static eShellError cmdHelp(shell_cmd_args *args) {
	...
	for (i = 0; i < ShellCmds1.count; i++) {
		sensactcore::Console::Writeln("%s, %s", ShellCmds1.cmds[i].cmd,
				ShellCmds1.cmds[i].desc);
	}

	sensactcore::Console::Writeln((const char *) "OK");
...
static eShellError cmdArgt(shell_cmd_args *args) {
	...
	sensactcore::Console::Writeln((char *) "OK\n\rargs given:");

	for (i = 0; i < args->count; i++) {
		sensactcore::Console::Writeln(" - %s", args->args[i].val);
	}
...
static eShellError cmdCs(shell_cmd_args *args) {
	...
	for (i = 1; i < (uint16_t) eApplicationID::CNT; i++) {
		...
		if(cShell::strcmp(args->args[0].val, MODEL::ApplicationNames[i], cmdNameLength, testCmdNameLength)==0)
		{
			...
			//TODO update shell cMaster::SendCommandToMessageBus(BSP::GetSteadyClock(), appId, (eCommandType)command, NULL, 0);
		}
	}
	return eShellError::PROCESS_OK;
}
...
static eShellError cmdSettime(shell_cmd_args *args) {
	...
	if(std::abs(int64_t(time - systemClockMsecCnt))>3) //only set the clock, if error is significant
	{
		...
		sensactcore::Console::Writeln("Set the clock to a new value");
	}
	return eShellError::PROCESS_OK;
}
...
static eShellError cmdSEND_CAN(uint8_t *cmdBuffer, const uint16_t size)
{
		...
		//TODO fix shell sensact::cMaster::SendCommandToMessageBus(BSP::GetSteadyClock(), (sensact::eApplicationID)appId, (sensact::eCommandType)commandId, (uint8_t*)&cmdBuffer[3], (uint8_t)(size-3));
		return eShellError::PROCESS_OK;
}
...
static eShellError cmdSEND_RAW_CAN(uint8_t *cmdBuffer, uint8_t size)
{
	...
	//todo fix shell sensact::BSP::SendCANMessage(id, buf, i);
	return eShellError::PROCESS_OK;
}
