...
ADK_Machine_TypeDef ADK_Machine __attribute__ ((aligned (4)));
...
static void USBH_ADK_InterfaceDeInit  (USBH_HandleTypeDef *phost);
...
{
    "ADK",
	0x00,//Device class = "use class info in interfaces
...
/**
  ...
  * @retval None
  */
void USBH_ADK_InterfaceDeInit (USBH_HandleTypeDef *phost)
{
#ifdef DEBUG
...
//	}
	...
	{
		USB_OTG_HC_Halt(phost, ADK_Machine.hc_num_out);
	...
	switch (ADK_Machine.state)
	{
		...
		case ADK_SEND_DATA:
			...
			if(ADK_Machine.outSize > 0){
				USBH_BulkSendData(phost, ADK_Machine.outbuff, ADK_Machine.outSize, ADK_Machine.hc_num_out);
				ADK_Machine.outSize = 0;
		...
		case ADK_GET_DATA:
			URB_Status = HCD_GetURB_State(phost , ADK_Machine.hc_num_in);
			HCD_Status = HCD_GetHCState(phost , ADK_Machine.hc_num_in);
			HCD_GXferCnt = HCD_GetXferCnt(phost , ADK_Machine.hc_num_in);
...
static USBH_StatusTypeDef USBH_ADK_getProtocol (USBH_HandleTypeDef* phost)
{
	...
	/* Control Request */
	return USBH_CtlReq(phost, &ADK_Machine.protocol , 2 );
}
...
{
	uint32_t xfercount;
	xfercount = HCD_GetXferCnt(phost, ADK_Machine.hc_num_in);
	if( xfercount > 0 ){
