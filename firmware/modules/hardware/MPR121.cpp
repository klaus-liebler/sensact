#include <modules/hardware/MPR121.h>
/* ------------- Local defines --------------- */
...
#define MPR121_ADDRESS_BYTE	0xB5
...
bool cMPR121::Setup( void )
{
	...
	if(HAL_I2C_IsDeviceReady(i2c, MPR121_ADDRESS_BYTE, 3, 1000)!=HAL_OK)
	{
		return false;
	}
	...
	if (success == true)
	{
		...
		writeRegister(NCLR, 0x10);
		writeRegister(FDLR, 0x20);
		writeRegister(MHDF, 0x01);
		writeRegister(NHDF, 0x01);
		writeRegister(NCLF, 0x10);
		writeRegister(FDLF, 0x20);
		writeRegister(NHDT, 0x01);
		writeRegister(NCLT, 0x10);
		writeRegister(FDLT, 0xFF);
		writeRegister(MHDPROXR, 0x0F);
		writeRegister(NHDPROXR, 0x0F);
		...
		writeRegister(DTR, 0x11);
		writeRegister(AFE1, 0xFF);
		writeRegister(AFE2, 0x30);
		writeRegister(ACCR0, 0x00);
		writeRegister(ACCR1, 0x00);
		writeRegister(USL, 0x00);
		writeRegister(LSL, 0x00);
		writeRegister(TL, 0x00);
		writeRegister(ECR, 0xCC); // default to fast baseline startup and 12 electrodes enabled, no prox

		/* apply next setting for all electrodes */
		for (electrodes_count = 0; electrodes_count < NUM_OF_ELECTRODES; electrodes_count++) {
			writeRegister((E0TTH + (electrodes_count<<1)), 40);
			writeRegister((E0RTH + (electrodes_count<<1)), 20);
		}

		/* enable electrodes and set the current to 16uA */
		writeRegister(ECR, 0x10);
	}
	else
	{
		/* init error */
...
bool cMPR121::writeRegister(uint8_t reg_addr, uint8_t value)
{

	return HAL_I2C_Mem_Write(i2c, (uint16_t)MPR121_ADDRESS_BYTE, (uint16_t)reg_addr, I2C_MEMADD_SIZE_8BIT, &value, 1, 1000)==HAL_OK;
...
bool cMPR121::readRegister(uint8_t reg_addr, uint8_t *value_ptr)
{
	return HAL_I2C_Mem_Read(i2c, (uint16_t)MPR121_ADDRESS_BYTE, (uint16_t)reg_addr, I2C_MEMADD_SIZE_8BIT, value_ptr, 1, 1000)==HAL_OK;
}
}
