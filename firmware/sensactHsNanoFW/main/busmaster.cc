...
namespace sensact
{
	...
	*/
	...
	ErrorCode cBusmaster::Init() const
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (line == UINT16_MAX)
				continue; // UINT16_MAX means: not defined
	...
	void cBusmaster::Process(Time_t now) const
	{
		...
		{
			if (interruptlines[i] == UINT16_MAX)
				continue;
	...
	cOwSubbus::cOwSubbus(
		...
		uint32_t const ds2413NodesCnt) : driver(driver),
	...
	bool cOwSubbus::GetSensactSeScratchpad(uint16_t index, uint8_t *airQuality, uint8_t *humidity, int16_t *temperatureX16, uint8_t *payloadSpec, uint16_t *payload)
	{
		...
		if (airQuality != 0)
			*airQuality = sensactSENodes[index].scratchpad[1];
		if (humidity != 0)
			*humidity = sensactSENodes[index].scratchpad[2];
		if (temperatureX16 != 0)
			*temperatureX16 = Common::ParseInt16(sensactSENodes[index].scratchpad, 3);
		if (payloadSpec != 0)
			*payloadSpec = sensactSENodes[index].scratchpad[5];
		if (payload != 0)
			*payload = Common::ParseInt16(sensactSENodes[index].scratchpad, 6);
		return true;
	}
	...
	// bei binären Ausgängen bestimmt das oberste Bit, ob an oder aus
