...
namespace sensacthal
{
    ...
    private:
        ...
        inline static void Modify2Bits(uint32_t *reg, uint8_t indexInRegister, uint32_t val)
        {
            indexInRegister &= 0xF;
            indexInRegister *= 2;
            uint32_t clearer = ~(indexInRegister << 0x3);
        ...
        inline static void Modify4Bits(uint32_t *reg, uint8_t indexInRegister, uint32_t val)
        {
            indexInRegister &= 0x7;
            indexInRegister *= 4;
            uint32_t clearer = ~(indexInRegister << 0xF);
            __IO uint32_t regVal = *reg;
            regVal &= clearer;
        ...
        {
            return 20 + 4 * ((((uint32_t)pin) & 0x08) >> 3);
        }
    ...
    public:
        static void ConfigureGPIOInput(Pin pin, PullDirection pulldir = PullDirection::NONE)
        {
            ...
            Modify2Bits(GPIOx_BASE + GPIOx_MODER, pinIndex, (uint32_t)Mode::Input); //GPIOx_MODER
            Modify2Bits(GPIOx_BASE + GPIOx_PUPDR, pinIndex, (uint32_t)pulldir);     //GPIOx_PUPDR
        }
        static void ConfigureGPIOOutput(Pin pin, bool initialValue, OutputType ot = OutputType::PUSH_PULL, OutputSpeed speed = OutputSpeed::LOW, PullDirection pulldir = PullDirection::NONE)
        {
            ...
            uint8_t pinIndex = pin2localPin(pin);
            uint32_t *GPIOx_BASE = pin2GPIOx_BASE(pin);
            Modify2Bits(GPIOx_BASE + GPIOx_OTYPER, pinIndex, (uint32_t)ot);          //GPIOx_OTYPER
            Modify2Bits(GPIOx_BASE + GPIOx_OSPEEDR, pinIndex, (uint32_t)speed);      //GPIOx_OSPEEDR
            Modify2Bits(GPIOx_BASE + GPIOx_PUPDR, pinIndex, (uint32_t)pulldir);      //GPIOx_PUPDR
            Modify2Bits(GPIOx_BASE + GPIOx_MODER, pinIndex, (uint32_t)Mode::Output); //GPIOx_MODER
        }
        static void ConfigureAlternateFunction(Pin pin, uint32_t GPIO_AFx_yyyy, OutputType ot = OutputType::PUSH_PULL, OutputSpeed speed = OutputSpeed::LOW, PullDirection pulldir = PullDirection::NONE)
        {
            ...
            uint8_t pinIndex = pin2localPin(pin);
            uint32_t *GPIOx_BASE = pin2GPIOx_BASE(pin);
            Modify2Bits(GPIOx_BASE + GPIOx_MODER, pinIndex, (uint32_t)Mode::AlternateFunction); //GPIOx_MODER
            Modify2Bits(GPIOx_BASE + GPIOx_OTYPER, pinIndex, (uint32_t)ot);                     //GPIOx_OTYPER
            Modify2Bits(GPIOx_BASE + GPIOx_OSPEEDR, pinIndex, (uint32_t)speed);                 //GPIOx_OSPEEDR
            Modify2Bits(GPIOx_BASE + GPIOx_PUPDR, pinIndex, (uint32_t)pulldir);                 //GPIOx_PUPDR
            Modify4Bits(GPIOx_BASE + pin2AFRxOffset(pin), pinIndex, GPIO_AFx_yyyy);             //GPIOx_PUPDR
        }
        static void ConfigureAnalog(Pin pin)
        {
            ...
            uint8_t pinIndex = pin2localPin(pin);
            uint32_t *GPIOx_BASE = pin2GPIOx_BASE(pin);
            Modify2Bits(GPIOx_BASE + GPIOx_MODER, pinIndex, (uint32_t)Mode::Analog); //GPIOx_MODER
        }
        ...
        static bool Get(Pin pin)
        {
            ...
            uint8_t pinIndex = pin2localPin(pin);
            uint32_t *GPIOx_BASE = pin2GPIOx_BASE(pin);
            return *(GPIOx_BASE + GPIOx_IDR) & (1 << pinIndex);
        }
    };
