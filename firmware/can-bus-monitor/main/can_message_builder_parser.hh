...
using namespace sensact::model::applications;
namespace sensact
{
	/*
	...
	*/
	...
	class cCANMessageBuilderParserOld:public aCANMessageBuilderParser{
		...
		void TraceCommandMessage(CANMessage &m) override{
            ...
            LOGI(TAG, "ApplicationCommand (old CAN-ID) to id 0x%02x (%s); command:0x%02x (%s); len:%d; payload: 0x%s", appId, appId<ApplicationNames.size()?ApplicationNames[appId]:"?", commandId, commandId<CommandNames.size()?CommandNames[commandId]:"?", payloadLen, charbuf);
		}
